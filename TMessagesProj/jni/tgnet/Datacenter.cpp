...
bool Datacenter::importPermanentAuthKey(const uint8_t *data, uint32_t length) {
    if (data == nullptr || length != 256) {
        return false;
    }

    clearAuthKey(HandshakeTypeAll);
    authKeyPerm = new ByteArray(const_cast<uint8_t *>(data), length);

    uint8_t digest[SHA_DIGEST_LENGTH];
    SHA1(authKeyPerm->bytes, authKeyPerm->length, digest);
    memcpy(&authKeyPermId, digest + SHA_DIGEST_LENGTH - sizeof(authKeyPermId), sizeof(authKeyPermId));

    authorized = true;
    lastInitVersion = 0;
    lastInitMediaVersion = 0;
    clearServerSalts(false);
    clearServerSalts(true);
    return true;
}
