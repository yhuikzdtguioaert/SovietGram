...
void ConnectionsManager::importAuthorizationKey(uint32_t datacenterId, std::string ipAddress, uint32_t port, std::vector<uint8_t> authKey) {
    scheduleTask([&, datacenterId, ipAddress, port, authKey] {
        if (datacenterId == 0 || authKey.size() != 256) {
            if (LOGS_ENABLED) DEBUG_E("account%u refused invalid imported authorization key for dc%u", instanceNum, datacenterId);
            return;
        }
        ...
        for (auto &entry : datacenters) {
            ...
            entry.second->clearAuthKey(HandshakeTypeAll);
            entry.second->recreateSessions(HandshakeTypeAll);
            entry.second->authorized = false;
        }
        ...
        if (!datacenter->importPermanentAuthKey(authKey.data(), static_cast<uint32_t>(authKey.size()))) {
            return;
        }
