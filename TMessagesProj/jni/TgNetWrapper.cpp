...
void importAuthorizationKey(JNIEnv *env, jclass c, jint instanceNum, jint datacenterId, jstring ipAddress, jint port, jbyteArray authKey) {
    if (authKey == nullptr) {
        return;
    }
    jsize keyLength = env->GetArrayLength(authKey);
    if (keyLength != 256) {
        return;
    }

    const char *valueStr = ipAddress != nullptr ? env->GetStringUTFChars(ipAddress, 0) : nullptr;
    std::vector<uint8_t> key(static_cast<size_t>(keyLength));
    env->GetByteArrayRegion(authKey, 0, keyLength, reinterpret_cast<jbyte *>(key.data()));

    ConnectionsManager::getInstance(instanceNum).importAuthorizationKey(
            static_cast<uint32_t>(datacenterId),
            valueStr != nullptr ? std::string(valueStr) : std::string(),
...
static JNINativeMethod ConnectionsManagerMethods[] = {
        ...
        {"native_importAuthorizationKey", "(IILjava/lang/String;I[B)V", (void *) importAuthorizationKey},
