...
public class WebRtcAudioRecord {
  ...
  /** captureType of a capture that carries only the device's own output, never the microphone. */
  private static final int CAPTURE_TYPE_SCREEN = 1;
    ...
    public void run() {
      ...
      while (keepAlive) {
        ...
        if (bytesRead == byteBuffer.capacity()) {
          ...
          if (keepAlive) {
            // Reshapes the microphone signal for Voice Changer. The native side reads this very
            // buffer through its cached address, so the frame is rewritten in place, and only a
            // capture that carries the microphone is touched.
            if (captureType != CAPTURE_TYPE_SCREEN) {
              VoiceChangerHelper.call().process(byteBuffer, bytesRead);
            }
            try {
  ...
  private boolean startRecording() {
    ...
    // The preset is read here rather than per frame, so switching voices mid-call takes effect on
    // the next call instead of splitting this one, and the negotiated format is what Sonic gets.
    if (captureType != CAPTURE_TYPE_SCREEN) {
      VoiceChangerHelper.call().start(audioRecord.getSampleRate(), audioRecord.getChannelCount());
    }
